...
%% NB -- add separate learning rates for rewards and punishments later
n_theta = 1; % single learning rate
f_name = @f_rev_Qlearn1; %Evolution function
g_name = @g_rev_softmax1; %Observation function
n_t = 80; %Total number of trials
% n_runs = 3; %3 blocks total
n_hidden_states = 2; %Track single value + PE
...
u(1,:) = subjects_actions; %Chosen action [1 2] -- CHECK!
u(2,:) = % reinforcement

u = [zeros(size(u,1),1) u(:,1:end-1)]; %Shift the u! This accommodates the initial values
...
% for i = 1:n_t % n_t -- number of trials, i -- current trial
%     try
...
% options.isYout = censor';
options.isYout = zeros(size(y)); % take all trials, consider excluding RT outliers later
options.inF.Yout = options.isYout;

%% Run the vba model
[posterior,out] = VBA_NLStateSpaceModel(y,u,f_name,g_name,dim,options);

%% Put a breakpoint for now, parse output later
